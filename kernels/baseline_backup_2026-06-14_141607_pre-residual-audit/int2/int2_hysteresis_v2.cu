...
__global__ void int2_hysteresis_v2_kernel(
    ...
    const int8_t threshold = (threshold_S < 1) ? 1 : ((threshold_S > 7) ? 7 : threshold_S);
    ...
    // Indirizzamento GLOBALE (vedi WRITE BACK): idx campo = base_riga*campi_per_byte*... + k
    const int w_gidx = n * packed_K * 4 + k;  // pack/unpack int2: 4 campi/byte
    const int h_gidx = n * packed_H * 2 + k;  // pack/unpack int4: 2 campi/byte
    int8_t q = bitpack::unpack_int2(W_packed, w_gidx);
    int8_t h = bitpack::unpack_int4(H_packed, h_gidx);
    ...
    float lr_scaled = lr * lr_scale * (float)threshold;
    ...
    h_new = max((int16_t)(-threshold), min((int16_t)threshold, h_new));

    // ========== 7. WRITE BACK ==========
    // L'atomico a 32 bit dentro pack_* DEVE allinearsi al buffer intero, non alla
    // riga: W_packed + n*packed_K NON è allineato a 4B se packed_K%4!=0 -> atomico
    // disallineato + cross-row (bug B1/B2/B17). Passando base globale + indice
    // globale, (data + (byte_idx & ~3)) cade sempre su una word 4B valida.
    bitpack::pack_int2(W_packed, w_gidx, q_new);
    bitpack::pack_int4(H_packed, h_gidx, (int8_t)h_new);
}
