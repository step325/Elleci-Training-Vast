...
extern "C" {
    ...
    // INT8 Tensor Core kernels (v2 — 2x throughput vs FP16 WMMA)
    void int2_matmul_int8_tc_v2(
        const int8_t* X_int8, const uint8_t* W_packed, half* Y,
        const float* d_scale_x, float gamma, int M, int N, int K,
        cudaStream_t stream
    );

    void int2_backward_dx_int8_tc_v2(
        const int8_t* dY_int8, const uint8_t* W_packed, half* dX,
        const float* d_scale_dy, float gamma, int M, int N, int K,
        cudaStream_t stream
    );
...
torch::Tensor tc_matmul(
    ...
    int M = X_flat.size(0);
    // K reale = feature di X (il kernel gestisce K%4!=0 con le guardie k<K).
    // NON ricostruire K=packed_K*4: sovrastima K quando K%4!=0 (bug B26).
    int K = X_flat.size(1);

    TORCH_CHECK((K + 3) / 4 == packed_K,
        "Dimension mismatch: X ha ", K, " feature ma W_packed ha packed_K=", packed_K,
        " (atteso ceil(K/4)=", (K + 3) / 4, ")");
...
void tc_hysteresis_step_v2(
    ...
    TORCH_CHECK(dW.dim() == 2, "dW deve essere 2D [N, K]");

    dW = dW.contiguous();
    int N = dW.size(0);
    int K = dW.size(1);

    // B19: layout dei buffer packed coerente con [N, K] (evita corruzione silenziosa
    // se il chiamante passa dW con shape/layout sbagliato).
    const int pK = (K + 3) / 4, pH = (K + 1) / 2;
    TORCH_CHECK(W_packed.dim() == 2 && W_packed.size(0) == N && W_packed.size(1) == pK,
        "W_packed atteso [", N, ", ", pK, "] = [N, ceil(K/4)]");
    TORCH_CHECK(H_packed.dim() == 2 && H_packed.size(0) == N && H_packed.size(1) == pH,
        "H_packed atteso [", N, ", ", pH, "] = [N, ceil(K/2)]");
    TORCH_CHECK(W_packed.dtype() == torch::kUInt8 && H_packed.dtype() == torch::kUInt8,
        "W_packed/H_packed devono essere uint8");
...
std::tuple<torch::Tensor, torch::Tensor> tc_quantize_activation(
    ...
    auto d_global_max = torch::zeros({1}, torch::dtype(torch::kFloat32).device(input.device()));
...
/**
 * INT2 matmul forward with INT8 Tensor Cores (v2)
 *
 * Requires pre-quantized X_int8 and device-side scale_x.
 * Use tc_quantize_activation_async() to obtain both before calling this.
 *
 * Y = X_int8 @ W.T * (scale_x / 127.0) * gamma
 */
torch::Tensor tc_matmul_int8(
    torch::Tensor X_int8,    // [*, K] int8
    torch::Tensor W_packed,  // [N, K/4] uint8
    torch::Tensor d_scale_x, // [1] float32 on device — from quantize_activation_async
    double gamma
) {
    TORCH_CHECK(X_int8.is_cuda(), "X_int8 must be a CUDA tensor");
    TORCH_CHECK(W_packed.is_cuda(), "W_packed must be a CUDA tensor");
    TORCH_CHECK(d_scale_x.is_cuda(), "d_scale_x must be a CUDA tensor");
    TORCH_CHECK(X_int8.dtype() == torch::kInt8, "X_int8 must be int8");
    TORCH_CHECK(W_packed.dtype() == torch::kUInt8, "W_packed must be uint8");
    TORCH_CHECK(d_scale_x.dtype() == torch::kFloat32, "d_scale_x must be float32");

    int N       = W_packed.size(0);
    int packed_K = W_packed.size(1);

    auto X_flat = X_int8.view({-1, X_int8.size(-1)});
    int M = X_flat.size(0);
    // K reale = feature di X (no ricostruzione packed_K*4, bug B26).
    int K = X_flat.size(1);
    ...
    // Output is FP16 (INT8 inputs, FP16 output after scaling)
    auto Y = torch::empty({M, N},
        torch::dtype(torch::kFloat16).device(X_int8.device()));

    cudaStream_t stream = c10::cuda::getCurrentCUDAStream();

    int2_matmul_int8_tc_v2(
        (const int8_t*)X_flat.data_ptr(),
        (const uint8_t*)W_packed.data_ptr(),
        (half*)Y.data_ptr(),
        (const float*)d_scale_x.data_ptr<float>(),
        (float)gamma,
        M, N, K,
        stream
    );

    auto out_shape = X_int8.sizes().vec();
    out_shape.back() = N;
    return Y.view(out_shape);
}

/**
 * INT2 backward dX with INT8 Tensor Cores (v2)
 *
 * Requires pre-quantized dY_int8 and device-side scale_dy.
 * Use tc_quantize_activation_async() to obtain both before calling this.
 *
 * dX = dY_int8 @ W * (scale_dy / 127.0) * gamma
 */
torch::Tensor tc_backward_input_int8(
    torch::Tensor dY_int8,    // [*, N] int8
    torch::Tensor W_packed,   // [N, K/4] uint8
    torch::Tensor d_scale_dy, // [1] float32 on device — from quantize_activation_async
    double gamma,
    int64_t K
) {
    TORCH_CHECK(dY_int8.is_cuda(), "dY_int8 must be a CUDA tensor");
    TORCH_CHECK(W_packed.is_cuda(), "W_packed must be a CUDA tensor");
    TORCH_CHECK(d_scale_dy.is_cuda(), "d_scale_dy must be a CUDA tensor");
    TORCH_CHECK(dY_int8.dtype() == torch::kInt8, "dY_int8 must be int8");
    TORCH_CHECK(d_scale_dy.dtype() == torch::kFloat32, "d_scale_dy must be float32");

    int N = W_packed.size(0);
    auto dY_flat = dY_int8.view({-1, dY_int8.size(-1)});
    int M = dY_flat.size(0);

    auto dX = torch::empty({M, K},
        torch::dtype(torch::kFloat16).device(dY_int8.device()));

    cudaStream_t stream = c10::cuda::getCurrentCUDAStream();

    int2_backward_dx_int8_tc_v2(
        (const int8_t*)dY_flat.data_ptr(),
        (const uint8_t*)W_packed.data_ptr(),
        (half*)dX.data_ptr(),
        (const float*)d_scale_dy.data_ptr<float>(),
        (float)gamma,
        M, N, (int)K,
        stream
    );

    auto out_shape = dY_int8.sizes().vec();
    out_shape.back() = K;
    return dX.view(out_shape);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("matmul_int8", &tc_matmul_int8,
        "INT8 matmul v2 with Tensor Cores: Y = X_int8 @ W.T * (scale_x/127) * gamma. "
        "2x throughput vs FP16 WMMA on A100.");

    m.def("backward_input_int8", &tc_backward_input_int8,
        "INT8 backward dX v2 with Tensor Cores: dX = dY_int8 @ W * (scale_dy/127) * gamma.");
}
