 ...
 * Uses per-tensor symmetric quantization: x_int8 = round(x * 127 / absmax)
 * dove absmax = max(abs(x)).
 *
 * NB (B22): lo scale ESPORTATO (scale_out) è l'absmax GREZZO, NON absmax/127.
 * I consumatori (matmul/backward INT8) applicano la normalizzazione come
 * (scale_out / 127.0). dequantize fa input_int8 * (scale_out / 127.0).
 *
...
__global__ void find_absmax_kernel(
    ...
    for (int idx = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        float raw = __half2float(input[idx]);
        float val = isfinite(raw) ? fabsf(raw) : 0.0f;
...
__global__ void quantize_kernel(
    ...
    for (int i = idx * 4; i < size - 3; i += stride * 4) {
        ...
        vals.z = isfinite(vals.z) ? vals.z : 0.0f;
        vals.w = isfinite(vals.w) ? vals.w : 0.0f;
...
void quantize_activation(
    ...
    float* d_global_max;
    CUDA_CHECK(cudaMallocAsync(&d_global_max, sizeof(float), stream));
    CUDA_CHECK(cudaMemsetAsync(d_global_max, 0, sizeof(float), stream));
    ...
    int quant_blocks = (size + BLOCK_SIZE * 4 - 1) / (BLOCK_SIZE * 4);
    quant_blocks = min(quant_blocks, 1024);

    // Pass 2: Quantize from device-side absmax; no host synchronization.
    quantize_kernel<<<quant_blocks, BLOCK_SIZE, 0, stream>>>(
        input, output, d_global_max, size
    );
    CUDA_CHECK(cudaGetLastError());

    CUDA_CHECK(cudaMemcpyAsync(scale_out, d_global_max, sizeof(float), cudaMemcpyDeviceToDevice, stream));

    CUDA_CHECK(cudaFreeAsync(d_global_max, stream));
}
...
// ===========================================================================
// BitNet v2 — Hadamard + INT4 Quantization Kernel (PLANNED)
// ===========================================================================
// Pure PyTorch implementation in int2_linear_tc.py is active.
// CUDA kernel below would provide ~3x speedup for activation quantization.
// To activate: implement hadamard_quantize_int4_kernel here,
// add binding in int2_tc_ops.cpp, then use int2_tc_ops.hadamard_quantize_int4
// in int2_linear_tc.py instead of the Python fallback.
// ===========================================================================
