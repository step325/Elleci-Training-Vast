 ...
 * Legacy fused path. The active fast path is int2_hysteresis_v2.cu.
 *
...
// Tile size for the M reduction loop
#define HYST_TILE_M 32
#define HYST_BLOCK_SIZE 256

/**
 * Fused hysteresis update kernel.
 *
 * Each block handles a tile of weights [n_start:n_end, k_start:k_end]
 * and each thread updates one packed weight/counter pair.
 */
__global__ void int2_hysteresis_optimized_kernel(
    ...
    // Indirizzamento GLOBALE (vedi WRITE BACK): allinea l'atomico al buffer intero.
    ...
    // ========== 7. WRITE BACK ==========
    // Atomico 32 bit allineato al buffer globale, non alla riga (bug B1/B2/B17).
