    ...
    if (byte_start + 3 < total_bytes) {
        ...
        if (col_start + WEIGHTS_PER_THREAD <= K) {
            ...
            for (int b = 0; b < 4; b++) {
                ...
                int8_t w0 = bitpack::decode_int2_bits(byte_val & 0x3);
                int8_t w1 = bitpack::decode_int2_bits((byte_val >> 2) & 0x3);
                int8_t w2 = bitpack::decode_int2_bits((byte_val >> 4) & 0x3);
                int8_t w3 = bitpack::decode_int2_bits((byte_val >> 6) & 0x3);
