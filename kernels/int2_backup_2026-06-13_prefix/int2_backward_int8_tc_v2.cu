...
__device__ __forceinline__ void expand_int2_x4_int8_bw(uint8_t packed, int8_t out[4]) {
    out[0] = (int8_t)(((packed >> 0) & 0x3) - 1);
    out[1] = (int8_t)(((packed >> 2) & 0x3) - 1);
    out[2] = (int8_t)(((packed >> 4) & 0x3) - 1);
    out[3] = (int8_t)(((packed >> 6) & 0x3) - 1);
}
...
__global__ void int2_backward_dx_int8_tc_v2_simple_kernel(
    ...
    int32_t sum = 0;
    for (int n = 0; n < N; n++) {
        int8_t w = (int8_t)(((W_packed[n * packed_K_dim + k/4] >> ((k%4)*2)) & 0x3) - 1);
...
static inline void _bwd_ensure_device_props() {
    if (s_bwd_num_sms >= 0) return;
    cudaDeviceProp prop;
...
void int2_backward_dx_int8_tc_v2(
    ...
    if (blocks_large >= s_bwd_num_sms * 2 && M >= BLOCK_M && K >= BLOCK_N && N >= BLOCK_K) {
    ...
    } else if (blocks_med >= s_bwd_num_sms && M >= BLOCK_M_MED && K >= BLOCK_N_MED && N >= BLOCK_K_MED) {
