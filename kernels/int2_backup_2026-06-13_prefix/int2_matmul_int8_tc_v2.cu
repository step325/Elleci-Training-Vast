...
__global__ void int2_matmul_int8_tc_v2_simple_kernel(
    ...
    int32_t sum = 0;
    for (int k = 0; k < K; k++) {
        int8_t w = (int8_t)(((W_packed[n * packed_K + k/4] >> ((k%4)*2)) & 0x3) - 1);
...
static inline void _fwd_ensure_device_props() {
    if (s_fwd_num_sms >= 0) return;
    cudaDeviceProp prop;
...
void int2_matmul_int8_tc_v2(
    ...
    if (blocks_large >= s_fwd_num_sms * 2 && M >= BLOCK_M && N >= BLOCK_N && K >= BLOCK_K) {
    ...
    } else if (blocks_med >= s_fwd_num_sms && M >= BLOCK_M_MED && N >= BLOCK_N_MED && K >= BLOCK_K_MED) {
