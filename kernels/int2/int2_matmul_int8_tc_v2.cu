...
#define SMEM_PAD_OUT  4   // stride out = (16+4)*4 = 80 bytes (multiplo di 16 ✓)
...
int2_matmul_int8_tc_v2_kernel(
    ...
    for (int k_base = 0; k_base < K; k_base += BLOCK_K) {
        ...
        // Carica X_int8 vettorizzato (16 bytes = 16 int8 per thread)
        #pragma unroll
        for (int i = tid; i < (BLOCK_M * BLOCK_K) / 16; i += THREADS_PER_BLOCK) {
            int m_l = (i * 16) / BLOCK_K;
            int k_l = (i * 16) % BLOCK_K;
            int m_g = block_m + m_l;
            int k_g = k_base + k_l;

            int4 val = {0, 0, 0, 0};
            // int4 (16B) richiede X_int8[m_g*K+k_g] allineato: k_g è multiplo di 16,
            // quindi serve K%16==0; altrimenti percorso scalare sicuro.
            if ((K % 16 == 0) && m_g < M && k_g + 15 < K) {
                val = *reinterpret_cast<const int4*>(&X_int8[m_g * K + k_g]);
            } else {
                __align__(16) int8_t tmp[16] = {0};
                for(int j=0; j<16; j++) {
                    if (m_g < M && k_g + j < K) tmp[j] = X_int8[m_g * K + k_g + j];
                }
                val = *reinterpret_cast<int4*>(&tmp[0]);
            }
            *reinterpret_cast<int4*>(&X_smem[m_l][k_l]) = val;
        }

        // Espandi W_packed vettorizzato (4 bytes = 16 pesi per thread)
        #pragma unroll
        for (int i = tid; i < (BLOCK_N * BLOCK_K) / 16; i += THREADS_PER_BLOCK) {
            int n_l    = i / (BLOCK_K / 16);
            int byte_chunk_l = i % (BLOCK_K / 16);
            int n_g    = block_n + n_l;
            int byte_g_start = (k_base / 4) + byte_chunk_l * 4;
            ...
            for(int b=0; b<4; b++) {
                int8_t exp[4];
                expand_int2_x4_int8(bytes[b], exp);
                int k_bl = (byte_chunk_l * 4 + b) * 4;
                #pragma unroll
                for (int j=0; j<4; j++) {
                    if (n_l < BLOCK_N && k_bl + j < BLOCK_K)
    ...
    for (int mi = 0; mi < 2; mi++) {
        ...
        for (int ni = 0; ni < 2; ni++) {
            ...
            // R1: il read-back delle lane sul tile (mi,ni) precedente deve completare
            // prima di riscrivere out_tiles[warp_id] (WAR su shared mem — racecheck).
            __syncwarp();
            ...
            int m_o = out_m + row;
            int n_o = out_n + col_chunk * 8;

            // float4 (16B) su Y richiede N%8==0 (n_o è già multiplo di 8).
            if ((N % 8 == 0) && m_o < M && n_o + 7 < N) {
                half out_vec[8];
                #pragma unroll
                for (int i = 0; i < 8; i++) {
                    out_vec[i] = __float2half((float)out_tiles[warp_id][row][col_chunk * 8 + i] * out_scale);
                }
                *reinterpret_cast<float4*>(&Y[m_o * N + n_o]) = *reinterpret_cast<float4*>(&out_vec[0]);
            } else {
                for (int i = 0; i < 8; i++) {
                    if (m_o < M && n_o + i < N) {
                        Y[m_o * N + n_o + i] = __float2half((float)out_tiles[warp_id][row][col_chunk * 8 + i] * out_scale);
                    }
                }
            }
        }
...
int2_matmul_int8_tc_v2_medium_kernel(
    ...
    for (int k_base = 0; k_base < K; k_base += BLOCK_K_MED) {
        ...
        // Espandi W_packed vettorizzato (4 bytes = 16 pesi per thread)
        #pragma unroll
        for (int i = tid; i < (BLOCK_N_MED * BLOCK_K_MED) / 16; i += THREADS_MED) {
            int n_l    = i / (BLOCK_K_MED / 16);
            int byte_chunk_l = i % (BLOCK_K_MED / 16);
            int n_g    = block_n + n_l;
            int byte_g_start = (k_base / 4) + byte_chunk_l * 4;
            ...
            for(int b=0; b<4; b++) {
                ...
                #pragma unroll
                for (int j=0; j<4; j++) {
                    if (n_l < BLOCK_N_MED && k_bl + j < BLOCK_K_MED)
                        W_smem[k_bl + j][n_l] = exp[j];
                }
            }
        }
