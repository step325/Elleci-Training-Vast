...
__global__ void mamba2_ssd_bwd_kernel(
    ...
    int h = blockIdx.y;
    int c_idx = blockIdx.z;
    int r     = threadIdx.x;  // 0..CHUNK_SIZE-1
    int d_tid = threadIdx.y;  // 0..D_THREADS-1
    ...
    // Tutti i thread leggono dt_val localmente (serve per dx_acc *= dt_val)
    float dt_val = (float)dt[dt_base + r * stride_dt_cs + h];

    // Solo d_tid==0 scrive dt_cumsum_shared
    __shared__ float dt_cumsum_shared[CHUNK_SIZE];
    if (d_tid == 0) {
    ...
    // Shared memory per riduzione ddt su d_tid
    __shared__ float ddt_reduce[CHUNK_SIZE][D_THREADS];

    float ddt_partial = 0.0f;

    // Loop parallelo su d con stride D_THREADS
    for (int d = d_tid; d < head_dim; d += D_THREADS) {
        float dx_acc  = 0.0f;
        float ddt_acc = 0.0f;
    ...
    // Riduzione di ddt_partial su tutti i d_tid dello stesso r
    ddt_reduce[r][d_tid] = ddt_partial;
    __syncthreads();

    if (d_tid == 0) {
        float total = 0.0f;
        for (int i = 0; i < D_THREADS; ++i) total += ddt_reduce[r][i];
        atomicAdd(&ddt[dt_base + r * stride_dt_cs + h], (scalar_t)total);
    }
}
...
torch::Tensor mamba2_ssd_fwd_cuda_launcher(
    ...
    // → CHUNK_SIZE * D_THREADS = 64 * 16 = 1024 thread/block (A100 max)
...
std::vector<torch::Tensor> mamba2_ssd_bwd_cuda_launcher(
    ...
    // Shared mem extern: B+C (2 * CS * P)
    // Shared mem statica: ddt_reduce (CS * D_THREADS) + dt_cumsum (CS) — allocate esternamente
