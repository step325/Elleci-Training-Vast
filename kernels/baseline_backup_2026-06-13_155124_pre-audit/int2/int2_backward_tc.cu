...
int2_backward_dx_tc_kernel(
    ...
    for (int mi = 0; mi < 2; mi++) {
        ...
        for (int ki = 0; ki < 2; ki++) {
            ...
            if (m_out < M && k_out + 7 < K) {
                *reinterpret_cast<float4*>(&dX[m_out * K + k_out]) = *reinterpret_cast<float4*>(&out_tiles[warp_id][row][col_chunk * 8]);
            } else {
                for (int i = 0; i < 8; i++) {
                    if (m_out < M && k_out + i < K) {
                        dX[m_out * K + k_out + i] = out_tiles[warp_id][row][col_chunk * 8 + i];
                    }
                }
            }
        }
