...
int2_matmul_tc_kernel(
    ...
    for (int mi = 0; mi < 2; mi++) {
        ...
        for (int ni = 0; ni < 2; ni++) {
            ...
            if (m_out < M && n_out + 7 < N) {
                *reinterpret_cast<float4*>(&Y[m_out * N + n_out]) = *reinterpret_cast<float4*>(&out_tiles[warp_id][row][col_chunk * 8]);
            } else {
                for (int i = 0; i < 8; i++) {
                    if (m_out < M && n_out + i < N) {
                        Y[m_out * N + n_out + i] = out_tiles[warp_id][row][col_chunk * 8 + i];
                    }
                }
            }
        }
